...
function Torrent (obj, namespace) {
  ...
  let torrentMeta = parseTorrent(this.torrentMetaBuffer)
  ...
  this.urlList = torrentMeta.urlList
  ...
  this.files = torrentMeta.files.map(f => new File(this, f))
...
Torrent.prototype.destroy = function () {
  this.close()
  indexedDB.deleteDatabase(this.infoHash)

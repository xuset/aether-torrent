...
Seeder.prototype._seed = function (torrent) {
  ...
  self._webtorrent.add(torrent.torrentMetaBuffer, opts, function (wtorrent) {
    ...
    if (wtorrent.urlList.length === 0) {
      const isSingleFile = wtorrent.files.length === 1
      let url = new URL(torrent.rootUrl)
      if (isSingleFile) url.pathname = path.join(url.pathname, 'planktos/files', wtorrent.files[0].name)
      wtorrent.addWebSeed(url.toString())
    }
